<div>
        ...
        <table class="w-full table-auto text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @foreach ($types as $type)
                    <tr wire:key="type-{{ $type->id }}" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 "
                        x-data
                        ...
                        <td class="px-6 py-4 text-right flex gap-2">
                            ...
                            <button 
                                ...
                                title="Editar"
                                wire:click.prevent="viewEditType({{ $type }})"
                            >
                            ...
                            <button 
                                ...
                                title="Excluir"
                                x-data
                                @click="if (confirm('Deseja realmente excluír este registro?')) { $wire.deleteType({{ $type }}) }"
                            >
    ...
    <x-modal name="edit-type">
        @include('type_associate.partials.update-type')
    </x-modal>
    <x-modal name="store-type">
        @include('type_associate.partials.store-type')
    </x-modal>
</div>