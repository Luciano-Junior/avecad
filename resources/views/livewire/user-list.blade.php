                ...
                @foreach ($users as $user)
                    <tr wire:key="user-{{ $user->id }}" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 "
                        ...
                        <td class="px-6 py-4 text-right flex gap-2">
                            @if ($user->id !== auth()->id())
                            <button class="hover:underline" title="{{$user->active?'Desativar':'Ativar'}}"
                                x-data
                                @click="if (confirm('Deseja realmente Inativar/Ativar este usuário?')) { $wire.toggleStatus({{ $user }}) }"
                                >
                                @if ($user->active)
                                <svg xmlns="http://www.w3.org/2000/svg" class="size-4" width="800px" height="800px" viewBox="0 0 24 24" fill="none">
                            ...
                            @endif
                            <x-nav-link-table :active="true" :href="route('user.edit', $user)" class="hover:underline" title="Editar">
